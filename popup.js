document.addEventListener("DOMContentLoaded", function () {
  chrome.storage.local.get(null, (items) => {
    ...
    Object.keys(groupedByDate).sort((a, b) => new Date(b) - new Date(a)).forEach(date => {
      ...
      let totalMinutes = entries.reduce((sum, { value }) => sum + value / 6, 0); // Calculating minutes
      ...
      let sectionHours = Math.floor(totalMinutes/60) + (Math.floor(totalMinutes/60) == 1 ? ' hour' : ' hours')
      let sectionMinutes = Math.floor(totalMinutes % 60) + (Math.floor(totalMinutes % 60) == 1 ? ' minute' : ' minutes')
      ...
      entries.forEach(({ key, value }) => {
        ...
        sectionHTML += `
          ...
          <div style="background: rgba(0, 150, 136, 0.8); width: ${barWidth.toFixed(2)}%; height: 16px; border-radius: 8px;"></div>
